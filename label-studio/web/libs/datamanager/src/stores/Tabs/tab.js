...
import {
  FF_ANNOTATION_RESULTS_FILTERING,
  isFF,
...
export const Tab = types
  ...
  .views((self) => ({
    ...
    get currentFilters() {
      return self.filters.filter((f) => {
        ...
        const annotationResultsOK =
          isFF(FF_ANNOTATION_RESULTS_FILTERING) ||
          !f.field.isAnnotationResultsFilterColumn;
  ...
  .actions((self) => ({
    ...
    applyChildFilter(rootFilter) {
      ...
      const firstChildColumn = self.targetColumns.find(
        (c) => c.alias === childFilter,
      );

      if (firstChildColumn && !rootFilter.child_filter) {
        const filterType = self.availableFilters.find(
          (ft) => ft.field.id === firstChildColumn.id,
        );

        if (filterType) {
          const childFilter = self.createChildFilterForType(
            filterType,
            rootFilter,
          );
        }
  ...
  .preProcessSnapshot((snapshot) => {
    ...
    if (agreement_selected) {
      Object.assign(sn, {
        agreement_selected:
          typeof agreement_selected === "string"
            ? JSON.parse(agreement_selected)
            : agreement_selected,
      });
    }
