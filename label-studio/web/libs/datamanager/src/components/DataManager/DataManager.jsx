...
const summaryInjector = inject(({ store }) => {
  ...
  console.log("lm------summaryInjector--store", store);
  ...
  return {
    ...
    projectsStatusData: taskStore?.projectsStatusData, // 添加 projectsStatusData
  };
});
...
const ProjectSummary = summaryInjector((props) => {
  const { projectsStatusData } = props;
  return (
    <Space
      size="large"
      style={{
    ...
    >
      <span>
        <span className="mr-[10px]">
          总时长：{projectsStatusData?.total_duration ?? "-"}分钟
        </span>
        <span className="mr-[10px]">
          总段落时长：{projectsStatusData?.total_region_duration ?? "-"}分钟
        </span>
        <span>总段落数：{projectsStatusData?.total_region_count ?? "-"}条</span>
      </span>
      <span>
        {props.cloudSync && (
          <Space
        ...
        <span style={{ display: "flex", alignItems: "center", fontSize: 12 }}>
          <Space size="compact">
            ...
            <span>预测结果: {props.totalPredictions}</span>
          </Space>
        </span>
      </span>
    </Space>
