...
export class LSFWrapper {
  ...
  onUpdateTask = async (taskID, updateData) => {
    console.log("Updating task:", taskID, updateData, this.datamanager.store);
    ...
    try {
      ...
      // 2. 调用tasks接口获取更新后的任务列表
      // const tasksResponse = await this.datamanager.apiCall("tasks", {
      //   page: this.datamanager.store.taskStore.page,
      //   page_size: this.datamanager.store.taskStore.pageSize,
      //   project: this.project.id,
      //   view: this.datamanager.store.viewsStore.selected.id,
      // });

      // 3. 使用返回的数据更新任务存储中的任务列表
      // if (tasksResponse && tasksResponse.tasks) {
      //   this.datamanager.store.taskStore.merge(tasksResponse.tasks);
      // }

      console.log("lm-------lsf----updateTask", taskID, result);
