...
#     print('annotation_templates_dir: ', annotation_templates_dir)
#     configs = []
...
class ProjectAnnotationStatsView(APIView):
    ...
    def get(self, request):
        ...
        if not project_id:
            return Response({"error": "Missing 'project' parameter"}, status=400)

        try:
            project_id = int(project_id)
        except (TypeError, ValueError):
            return Response({"error": "'project' must be an integer"}, status=400)
