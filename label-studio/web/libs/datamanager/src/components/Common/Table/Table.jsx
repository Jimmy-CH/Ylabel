...
import { AutoSizerTable, Tooltip, Button, Loading } from "@humansignal/ui";
...
export const Table = observer(
  ({
      ...
      return (
        ...
        >
          ...
          <DensityToggle
            ...
            data-testid="density-toggle-quickview"
          />
        </div>
      );
    ...
    return (
      <div
        ref={tableWrapper}
        className={tableCN.mod({ fit: props.fitToContent }).toString()}
      >
        {isQuickView && renderTableToolbar()}
        <TableContext.Provider value={contextValue}>
          {(store?.dataStore?.loading || store?.currentView?.locked) && (
            <Loading
              style={{
                position: "absolute",
                width: "100%",
  ...
  return (
    <StickyListContext.Consumer>
      {({
        ...
        const topPosition = isQuickView
          ? Math.max(0, Math.min(toolbarHeight, headerTopOffset ?? 0))
          : 0;
        ...
        const childrenArray = Array.isArray(children)
          ...
          : children
            ...
            : [];
        const toolbar =
          isQuickView && childrenArray.length > 0 ? childrenArray[0] : null;
        const bodyRows =
          isQuickView && childrenArray.length > 0
        ...
        return (
          <div ref={ref} {...rest}>
            ...
            {isQuickView ? (
              <>
                {toolbar}
                <div className={tableCN.elem("body-rows").toString()}>
                  {bodyRows}
                </div>
              </>
            ) : (
          ...
          <Button
            ...
            leading={
              <Icon
                ...
                style={{ color: "var(--color-neutral-content-subtle)" }}
              />
            }
          />
        </Tooltip>
        <pre
          className="m-0 whitespace-pre-wrap break-words max-w-full"
