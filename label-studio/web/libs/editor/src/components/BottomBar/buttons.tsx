...
export const CheckStatusButton = memo(
  observer(({ store }: CheckStatusButtonProps) => {
    const dropdownRef = useRef<DropdownRef>() as RefObject<DropdownRef>;
    ...
    return (
      <>
        <DropdownTrigger
          ...
          animated
          ref={dropdownRef}
          content={<CheckStatusForm ref={dropdownRef} store={store} />}
          enabled
          onVisibilityChanged={(visible) => {
            setIsVisible(visible);
          }}
        >
          <Button trailing={isVisible ? <IconChevron /> : <IconChevronDown />}>
            审核
          </Button>
        </DropdownTrigger>
      </>
        ...
        {showInfoIcon && (
          <Tooltip title="Annotators and Reviewers will not be able to skip this task">
            <IconInfoOutline
              width={20}
              height={20}
              className="text-neutral-content ml-auto cursor-pointer"
            />
          </Tooltip>
        )}
        <Button
