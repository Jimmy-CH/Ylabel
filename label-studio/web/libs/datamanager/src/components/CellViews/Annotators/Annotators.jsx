...
export const Annotators = (cell) => {
  ...
  const sdk = useSDK();
  const maxUsersToDisplay =
    window.APP_SETTINGS.data_manager?.max_users_to_display ?? 0;
  const userList =
    maxUsersToDisplay > 0
      ? Array.from(value).slice(0, maxUsersToDisplay)
      : value;
  ...
  const extraCount = useMemo(() => {
    ...
    column.alias,
    task?.annotators_count,
    task?.reviewers_count,
    task?.comment_authors_count,
  ]);
    ...
    <div className={annotatorsCN.toString()}>
      {userList.map((item, index) => {
        ...
        const userpicIsFaded =
          ...
          (isDefined(reviewed) && reviewed === false && isEnterprise);
                  ...
                  bottomRight: review && (
                    <div
                      className={clsx(
                        ...
                        userPickBadge.mod({ [review]: true }).toString(),
                      )}
                    >
                      {review === "rejected" ? (
                        <IconCrossAlt />
