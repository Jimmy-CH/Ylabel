...
const Label = ({ label, template, color }) => {
  ...
  return (
    ...
    >
      <span
        className={cnm(configClass.elem("label-text").toClassName(), "flex")}
      >
        <label style={{ background: color }}>
          <Input
            ...
            onChange={(e) =>
              template.changeLabel(label, { background: e.target.value })
            }
          />
        </label>
        <span>{value}</span>
  ...
  return (
    <div className={configClass.elem("labels")}>
      <form className={configClass.elem("add-labels")} action="">
        <h4>{tagname === "Choices" ? "Add choices" : "添加标签名称"}</h4>
...
export const ConfigPage = ({
  ...
  const onSelectRecipe = React.useCallback((recipe) => {
    if (!recipe) {
      ...
      setMode("view");
      __lsa(
        `labeling_setup.view.${snakeCase(recipe.group)}.${snakeCase(recipe.title)}`,
      );
    }
  });
