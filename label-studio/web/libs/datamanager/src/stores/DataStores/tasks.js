  ...
  return DataStore("TasksStore", {
    ...
    .actions((self) => ({
      ...
      loadTask: flow(function* (taskID, { select = true } = {}) {
        ...
        console.log("lm-------taskData", taskData);
      ...
      updateTask: flow(function* (taskID, updateData) {
        if (!isDefined(taskID)) {
          console.warn("Task ID must be provided");
          return;
        }
        ...
        try {
          // 正确的方式：taskID 作为路径参数，更新数据作为请求体
          const taskData = yield self.root.apiCall("updateTask", {
            taskID, // 这是路径参数
            body: updateData, // 这是请求体数据
          });

          console.log(
            "lm-------query--111-----updateTask",
            { taskID, body: updateData },
            taskData,
          );
          ...
          const updatedTask = self.applyTaskSnapshot(taskData, taskID);
          self.finishLoading(taskID);
      ...
      loadNextTask: flow(function* ({ select = true } = {}) {
        ...
        if (isLabelStream) {
          const selectedAnnotationID =
            getRoot(self).annotationStore.selected?.id;
          console.log(
