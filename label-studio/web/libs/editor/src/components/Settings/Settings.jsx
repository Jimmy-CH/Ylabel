...
const HotkeysDescription = () => {
  const columns = [
    ...
    { title: "项目描述", dataIndex: "descr", key: "descr" },
  ];
    ...
    Object.keys(descr)
      ...
      .map((k) => ({
        ...
        combo: k.split(",").map((keyGroup) => {
          return (
            <div
              className={cn("keys").elem("key-group").toClassName()}
              key={keyGroup}
            >
              {keyGroup
                .trim()
  ...
  return (
    <div className={cn("settings").mod(newUI).toClassName()}>
      {editorSettingsKeys.map((obj, index) => {
        return (
          <label
            className={cn("settings").elem("field").toClassName()}
            key={index}
          >
            {isFF(FF_DEV_3873) ? (
              <>
