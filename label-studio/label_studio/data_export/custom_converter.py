...
from label_studio_sdk.converter.keypoints import process_keypoints_for_coco, build_kp_order, \
    update_categories_for_keypoints, keypoints_in_label_config, get_yolo_categories_for_keypoints
...
from data_export.utils import merge_labels_with_transcription
...
class Converter(object):
    ...
    def convert_to_json_min(self, input_data, output_dir, is_dir=True):
        ...
        with io.open(output_file, mode="w", encoding="utf8") as fout:
            ...
            for item in item_iterator(input_data):
                ...
                raw_labels = output_data.get("labels", [])
                raw_trans = output_data.get("transcription", [])
                merged_labels = merge_labels_with_transcription(raw_labels, raw_trans)
                if item.get("id") is not None:
                    record["id"] = item["id"]
                # 遍历其他字段（除了 labels 和 transcription）
                for name, value in output_data.items():
                    if name == "labels":
                        # 使用合并后的 labels
                        record["labels"] = prettify_result(merged_labels)
                    elif name == "transcription":
                        record["transcription"] = prettify_result(value)
                    else:
    ...
    def convert_to_yolo(
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            categories, category_name_to_id = process_and_save_yolo_annotations(labels, label_path, category_name_to_id,
                                                                                categories, is_obb, is_keypoints,
                                                                                self._schema)
