...
export const HotkeysHeaderButtons = () => {
  ...
  const { handleResetToDefaults, handleExportHotkeys, handleImportHotkeys } =
    useHotkeys();
...
export const HotkeysManager = () => {
  ...
  const [duplicateConfirmDialog, setDuplicateConfirmDialog] =
    useState<DuplicateConfirmDialog>({
      open: false,
      hotkeyId: null,
      newKey: null,
      conflictingHotkeys: [],
  ...
  const { hotkeys, setHotkeys, isLoading, setIsLoading, saveHotkeysToAPI } =
    useHotkeys();
  ...
  const getSectionTitle = (sectionId: string): string => {
    const section = typedHotkeySections.find(
    ...
    try {
      ...
      if (result.ok) {
        ...
        const sectionName =
          sectionId === "settings"
            ? "设置"
            : typedHotkeySections.find((s: Section) => s.id === sectionId)
                ?.title;
  ...
  return (
    <div id="hotkeys-manager">
              ...
              <Card key={section.id}>
                ...
                <CardContent>
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={`py-wide ${i < 3 ? "border-b border-border" : ""}`}
                    >
                      <Skeleton className="h-5 w-44 mb-tight" />
                      <Skeleton className="h-base w-[16rem]" />
                    </div>
      ...
      <Dialog
        ...
        onOpenChange={handleCancelDuplicate}
      >
        <DialogContent className="bg-neutral-surface">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>{duplicateConfirmDialog.newKey}</strong>" is already being
              used by:
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="max-h-60 overflow-y-auto">
            <div className="flex flex-col gap-base">
              {duplicateConfirmDialog.conflictingHotkeys.map(
                (conflictHotkey: Hotkey) => (
                  <div
          ...
          <DialogDescription className="text-warning-text bg-warning-background p-base rounded-small border border-warning-border-subtle flex items-start gap-tight">
            ...
            <div>
              Having duplicate hotkeys may cause conflicts and unexpected
              behavior. Are you sure you want to proceed?
            </div>
          </DialogDescription>
